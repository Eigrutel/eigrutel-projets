...
`index.html` est le fichier source principal. Modifiez-le pour les évolutions futures. `projets.html` est une redirection, à conserver pour les liens utilisant ce nom. Les favicons sont intégrés au HTML ; aucun dossier favicon supplémentaire n’est nécessaire à Projets. Chaque application distante conserve ses propres ressources dans son dépôt.
...
La version reste **0.13.0** pour cette étape. Une version stable 1.0.0 pourra être préparée après validation, avec mise à jour cohérente du HTML, du README, de la notice et de l’historique. Le fichier `RELEASE_NOTES.md` contient un texte prêt pour une éventuelle préversion 0.13.0.
...
No build step is required. `index.html` contains the complete hub; `projets.html` redirects to it. Icons are embedded. Other tools remain separately hosted.
